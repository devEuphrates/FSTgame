Shader "Unlit/GridVisualizerShader"
{
	Properties
	{
		...
		return o;
         ...
         float DrawGrid(float2 uv, float sz, float aa)
         {
			...
			{   
				...
				fixed r = DrawGrid(i.uv, _GridSize, 0.04);
				fixed b = DrawGrid(i.uv, _Grid2Size, 0.04);
				fixed g = DrawGrid(i.uv, _Grid3Size, 0.04);
				return float4(_Alpha,_Alpha,_Alpha,(r+b+g)*_Alpha);
			}
			ENDCG
		}
	}
